...
def _evidence_root() -> Path:
    """Return the mounted evidence package for the B1 forensic closure."""
    return require_evidence_root(
        "artifacts/b1_forensic/b1_nested_coverage.json",
        "artifacts/b1_forensic/failure_waterfall.csv",
        "artifacts/b1_forensic/zero_coverage_controlled.json",
        "artifacts/api_audit/twenty_session_availability_probe.json",
        "artifacts/b1_repair/first_failure_waterfall.csv",
        "artifacts/b1_repair/controlled_vs_pipeline_diff.csv",
        "artifacts/b1_repair/cache_key_audit.json",
    )


def _nested() -> dict:
    path = _evidence_root() / "artifacts/b1_forensic/b1_nested_coverage.json"
...
def test_failure_waterfall_has_all_stages_for_each_origin() -> None:
    path = _evidence_root() / "artifacts/b1_forensic/failure_waterfall.csv"
...
def test_controlled_zero_coverage_trace_is_explicit_and_reproducible() -> None:
    path = _evidence_root() / "artifacts/b1_forensic/zero_coverage_controlled.json"
...
def test_full_tape_route_remains_diagnostic_only() -> None:
    ...
    selection = (REPOSITORY_ROOT / "docs/b1_benchmark_selection.md").read_text(encoding="utf-8")
...
def test_literature_ledger_uses_conservative_status_vocabulary() -> None:
    ...
    path = REPOSITORY_ROOT / "docs/literature_evidence_ledger.csv"
    rows = list(csv.DictReader(path.open(encoding="utf-8")))
    assert len(rows) == 10
...
def test_twenty_session_probe_is_metadata_only() -> None:
    path = _evidence_root() / "artifacts/api_audit/twenty_session_availability_probe.json"
    data = json.loads(path.read_text(encoding="utf-8"))
...
def test_repair_waterfall_and_controlled_reconciliation_are_closed() -> None:
    repair = _evidence_root() / "artifacts" / "b1_repair"
    with (repair / "first_failure_waterfall.csv").open(encoding="utf-8") as handle:
    ...
    diff = list(csv.DictReader((repair / "controlled_vs_pipeline_diff.csv").open(encoding="utf-8")))
    assert len(diff) == 4
    assert all(row["first_divergent_stage"] == "none" for row in diff)
    cache = json.loads((repair / "cache_key_audit.json").read_text(encoding="utf-8"))
