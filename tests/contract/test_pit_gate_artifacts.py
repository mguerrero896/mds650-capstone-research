...
def _json(name: str) -> dict:
    return json.loads((PIT / name).read_text(encoding="utf-8"))


def test_target_horizon_remains_rv30_and_rv10_is_not_introduced() -> None:
    text = (ROOT / "docs" / "target_horizon_decision.md").read_text(encoding="utf-8")
...
def test_daily_common_history_v5_is_complete_bounded_and_sanitized() -> None:
    data = json.loads(
        (ROOT / "artifacts" / "api_audit" / "common_history_continuity_v5.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_canonical_forecast_origins_have_one_target_and_no_future_predictors() -> None:
    frame = pl.read_parquet(PIT / "canonical_forecast_origins.parquet")
...
def test_final_gate_report_is_fail_closed_and_sanitized() -> None:
    ...
    text = (PIT / "pit_gate_final_report.json").read_text(encoding="utf-8").lower()
