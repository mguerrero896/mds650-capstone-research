...
RESULTS = PHASE6 / "results.json"
PREDICTIONS = PHASE6 / "oos_predictions.parquet"
SENSITIVITY = PHASE6 / "oos_sensitivity_predictions.parquet"
STABILITY = PHASE6 / "stability_results.parquet"
LEDGER = PHASE6 / "oos_access_ledger.json"
HANDOFF = ROOT / "reports" / "CODEX_PHASE6_FINAL_HANDOFF.md"
DECISIONS = {
...
pytestmark = pytest.mark.skipif(
    not RESULTS.exists(), reason="Phase 6 OOS evaluation remains sealed"
)
...
def test_phase6_result_is_self_hashed_single_read_and_sanitized() -> None:
    ...
    assert ledger["results_sha256"] == _sha256(RESULTS)
...
def test_phase6_timing_and_stability_views_are_complete() -> None:
    sensitivity = pl.read_parquet(SENSITIVITY)
    stability = pl.read_parquet(STABILITY)
