...
def test_pit_grid_contains_primary_and_sensitivities_without_silent_imputation() -> None:
    grid = pl.read_parquet(ROOT / "artifacts" / "pit" / "pit_sensitivity_grid_v1.parquet")
    summary = pl.read_csv(ROOT / "artifacts" / "pit" / "pit_sensitivity_summary_v1.csv")
...
def test_phase4a_profile_records_zero_post_origin_and_no_duplicate_suffixes() -> None:
    profile = json.loads((COMMON / "common_matrix_profile_v1.json").read_text(encoding="utf-8"))
...
def test_reproducibility_manifest_separates_environment_telemetry() -> None:
    manifest = json.loads((ROOT / "artifacts" / "repository" / "reproducibility_manifest_v1.json").read_text(encoding="utf-8"))
