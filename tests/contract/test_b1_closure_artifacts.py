...
def test_common_history_has_all_assets_and_dates() -> None:
    payload = json.loads(
        (ROOT / "artifacts/api_audit/common_history_all_assets_v3.json").read_text(encoding="utf-8")
    )
...
def test_literature_matrix_has_ten_rows_and_stable_identifiers() -> None:
    with (ROOT / "docs/literature_matrix.csv").open(encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    assert len(rows) == 10
    assert len({row["study_id"] for row in rows}) == 10
    assert all(
        row["doi_or_stable_url"].startswith(("https://doi.org/", "https://"))
        for row in rows
    )
...
def test_cache_index_has_unique_idempotency_keys() -> None:
    payload = json.loads(
        (OUT / "massive_contract_day_cache/cache_index.json").read_text(encoding="utf-8")
    )
