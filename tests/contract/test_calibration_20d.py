...
def test_duplicate_central_directory_tail_is_read_without_mutating_raw(tmp_path) -> None:
    """A repeated ZIP directory must not corrupt reading or rewrite provider bytes."""
    path = tmp_path / "full_tape_2025-09-29.zip"
    payload = b"id,underlying_symbol\ntrade-1,AAPL\n"
    member = "2025-09-29-option_trades.csv"
    with zipfile.ZipFile(path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr(member, payload)
    original = path.read_bytes()
    with zipfile.ZipFile(path) as archive:
        duplicated = original + original[archive.start_dir :]
    path.write_bytes(duplicated)

    with downloader._open_zip(path) as archive:
...
def test_parquet_writer_reuses_one_writer_without_truncating_pages(tmp_path, monkeypatch) -> None:
    ...
    for writer in writers.values():
        writer.close()
    path = tmp_path / "date=2026-06-11" / "asset=AAPL" / "events.parquet"
    assert pq.ParquetFile(path).metadata.num_rows == 2
    assert pq.read_table(path).column("id").to_pylist() == ["first", "second"]


def test_required_artifacts_are_sanitized_when_present() -> None:
    path = ROOT / "artifacts/calibration_20d/download_manifest.json"
    if not path.exists():
        return
...
def test_raw_integrity_is_fail_closed_when_present() -> None:
    path = ROOT / "artifacts/calibration_20d/raw_integrity_report.json"
    if not path.exists():
        return
    data = json.loads(path.read_text(encoding="utf-8"))
...
def test_pilot_application_marks_unusual_label_as_secondary_exploratory() -> None:
    path = ROOT / "artifacts/calibration_20d/pilot_v2_unusual_scores.parquet"
    if not path.exists():
        return
