...
# The Phase 6 grid resolves both option types at every registered DTE and
# moneyness target; quote downloads remain bounded to those contract-days.
MONEY = (0.95, 0.975, 1.0, 1.025, 1.05)
QUOTE_PAGE_LIMIT = 50_000
# ponytail: one decoder prevents parallel Python object expansion from exhausting RAM.
_CACHE_READ_SEMAPHORE = threading.Semaphore(1)


def _read_cache_payload(path: Path) -> dict[str, Any]:
    """Decode one large cache while bounding concurrent memory expansion."""
    with _CACHE_READ_SEMAPHORE:
...
def bsm_price(
    ...
    d2 = d1 - sigma * root_t
    if kind == "call":
        return spot * math.exp(-dividend * time_years) * norm_cdf(d1) - strike * math.exp(
            -rate * time_years
        ) * norm_cdf(d2)
    return strike * math.exp(-rate * time_years) * norm_cdf(-d2) - spot * math.exp(
        -dividend * time_years
    ) * norm_cdf(-d1)
...
def invert_iv(
    ...
    if not lower <= midpoint <= upper or midpoint <= 0:
        return {
            "success": False,
            "failure_reason": "ARBITRAGE_BOUND",
            "lower_bound": lower,
            "upper_bound": upper,
            "iterations": 0,
        }
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        return (
            ...
            response.headers.get("x-request-id") or response.headers.get("request_id"),
        )
...
def resolve_contracts(
    ...
    for bucket, (lower_dte, upper_dte) in BUCKETS.items():
        ...
        status, payload, request_id = _request_json(
            client, "https://api.massive.com/v3/reference/options/contracts", params, key
        )
        if status != 200:
        ...
        while next_url:
            ...
            if page_status != 200:
                raise RuntimeError(
                    f"MASSIVE_CONTRACT_PAGE_HTTP_{page_status}:{asset}:{day}:{bucket}"
                )
        ...
        for row in candidates:
            ...
            for target in MONEY:
                ...
                if previous is None or abs(moneyness - target) < abs(
                    float(previous["strike"]) / spot - target
                ):
                    selected[key_name] = {
                        "contract": ticker,
    ...
    unique = {row["contract"]: row for row in selected.values()}
    return list(unique.values())


def _session_bounds_ns(day: str) -> tuple[int, int]:
    """Return the actual XNYS session bounds in UTC nanoseconds for ``day``.

    The exchange calendar, rather than a fixed 09:30--16:00 assumption,
    handles daylight-saving transitions and early closes (for example July 3).
    """
    ...
    return int(opened.timestamp() * 1_000_000_000), int(closed.timestamp() * 1_000_000_000)


def _validate_session_quotes(
    rows: list[dict[str, Any]], start_ns: int, end_ns: int, context: str = ""
) -> None:
    """Fail closed when Massive returns a quote outside the requested session."""
    outside = [
        int(row["sip_timestamp"])
        for row in rows
        if not start_ns <= int(row["sip_timestamp"]) <= end_ns
    ]
    if outside:
        raise RuntimeError(
            "MASSIVE_QUOTE_OUTSIDE_REQUESTED_SESSION:"
            f"{context}:requested={start_ns}-{end_ns}:"
            f"observed={min(outside)}-{max(outside)}"
        )
    if any(not isinstance(row.get("sequence_number"), int) for row in rows):
        raise RuntimeError("MASSIVE_QUOTE_SEQUENCE_NUMBER_MISSING")
    keys = [(row["sip_timestamp"], row["sequence_number"]) for row in rows]
    if len(keys) != len(set(keys)):
        ...
        for key in keys:
            if key in seen:
                ...
                break
            seen.add(key)
        raise RuntimeError(f"MASSIVE_QUOTE_DUPLICATE_EVENT:{context}:key={duplicate}")


def _deduplicate_quote_events(
    rows: list[dict[str, Any]], context: str
) -> tuple[list[dict[str, Any]], int]:
    """Remove only byte-equivalent provider duplicates by SIP event key."""
    unique: dict[tuple[int, int], dict[str, Any]] = {}
    removed = 0
    for row in rows:
        key = (row["sip_timestamp"], row["sequence_number"])
        ...
        if previous is None:
            ...
            raise RuntimeError(f"MASSIVE_QUOTE_EVENT_KEY_COLLISION:{context}:key={key}")
    return list(unique.values()), removed


def _legacy_cache_has_terminal_page(payload: dict[str, Any]) -> bool:
    """Infer a terminal page only when the raw row count was not page-aligned."""
    rows = payload.get("results", [])
    if not isinstance(rows, list):
        return False
    original_count = len(rows) + int(payload.get("provider_duplicate_rows_removed", 0))
    return original_count == 0 or original_count % QUOTE_PAGE_LIMIT != 0
...
def fetch_contract_day(item: tuple[str, str, dict[str, Any]], key: str) -> dict[str, Any]:
    ...
    start_ns, end_ns = _session_bounds_ns(day)
    cache_key = f"provider=massive|asset={asset}|session_date={day}|expiry={contract['expiry']}|strike={contract['strike']}|option_type={contract['option_type']}|contract={ticker}|route=B1Q|schema_version=4"
    ...
    if path.exists():
        try:
            ...
            corrupt_hash = hashlib.sha256(path.read_bytes()).hexdigest()[:16]
            path.replace(path.with_suffix(path.suffix + f".invalid-{corrupt_hash}"))
        else:
            data["results"] = _compact_quotes(data.get("results", []))
            if data.get("schema_version") != 4:
                raise RuntimeError(f"MASSIVE_QUOTE_CACHE_SCHEMA_MISMATCH:{path.name}")
            if data.get("http_status") != 200:
                raise RuntimeError(f"MASSIVE_QUOTE_CACHE_HTTP_INVALID:{path.name}")
            data["results"], removed = _deduplicate_quote_events(data["results"], ticker)
            data["provider_duplicate_rows_removed"] = (
                int(data.get("provider_duplicate_rows_removed", 0)) + removed
            )
            if data.get("pagination_complete") is not True:
                if not _legacy_cache_has_terminal_page(data):
                    cache_hash = hashlib.sha256(path.read_bytes()).hexdigest()[:16]
                    path.replace(
                        path.with_suffix(path.suffix + f".pagination-unverified-{cache_hash}")
                    )
                else:
                    data["pagination_complete"] = "INFERRED_TERMINAL_PARTIAL_PAGE"
            if path.exists():
                _validate_session_quotes(data["results"], start_ns, end_ns, ticker)
                data["cache_hit"] = True
    ...
    params = {
        ...
        "timestamp.lte": str(end_ns),
        "sort": "timestamp",
        "order": "asc",
        "limit": str(QUOTE_PAGE_LIMIT),
    }
    endpoint = f"https://api.massive.com/v3/quotes/{ticker}"
    with httpx.Client(timeout=90, follow_redirects=True) as client:
        status, payload, request_id = _request_json(client, endpoint, params, key)
    if status != 200:
        raise RuntimeError(f"MASSIVE_QUOTE_HTTP_{status}:{ticker}:{day}")
    source_request_hash = hashlib.sha256(
        f"https://api.massive.com/v3/quotes/{ticker}|{json.dumps(params, sort_keys=True)}|route=B1Q|schema_version=4".encode()
    ).hexdigest()
    safe: dict[str, Any] = {
        "asset": asset,
        "day": day,
        "contract": contract,
        "cache_key": cache_key,
        "quote_cache_key": f"provider=massive|contract={ticker}|session_date={day}|route=B1Q|schema_version=4",
        "route": "B1Q",
        "schema_version": 4,
        "http_status": status,
        ...
        "pages": 1,
        "results": _compact_quotes(payload.get("results", [])),
        "cache_hit": False,
    }
    ...
    while next_url:
        if next_url in seen_urls:
            raise RuntimeError("MASSIVE_QUOTE_PAGINATION_REPEATED")
        seen_urls.add(next_url)
        cursor = parse_qs(urlparse(next_url).query).get("cursor", [None])[-1]
        if not cursor:
            raise RuntimeError("MASSIVE_QUOTE_NEXT_URL_CURSOR_MISSING")
        with httpx.Client(timeout=90, follow_redirects=True) as client:
            page_status, page, _ = _request_json(
                client, endpoint, {**params, "cursor": cursor}, key
            )
        if page_status != 200:
            raise RuntimeError(f"MASSIVE_QUOTE_PAGE_HTTP_{page_status}:{ticker}:{day}")
        safe["pages"] += 1
        safe["results"].extend(_compact_quotes(page.get("results", [])))
        next_url = page.get("next_url")
    safe["pagination_complete"] = True
    safe["results"], removed = _deduplicate_quote_events(safe["results"], ticker)
    safe["provider_duplicate_rows_removed"] = removed
    _validate_session_quotes(safe["results"], start_ns, end_ns, ticker)
...
def latest_quote(cache: dict[str, Any], origin_ns: int) -> dict[str, Any] | None:
    ...
    order_keys = cache.get("_quote_order_keys")
    if not isinstance(timestamps, list) or not isinstance(order_keys, list):
        rows = sorted(
            (
                row
                for row in rows
            ...
            key=lambda row: (
                row["sip_timestamp"],
                row.get("sequence_number", -1),
            ),
        )
        timestamps = [row["sip_timestamp"] for row in rows]
        order_keys = [(row["sip_timestamp"], row.get("sequence_number", -1)) for row in rows]
        cache["results"] = rows
        cache["_sip_timestamps"] = timestamps
        cache["_quote_order_keys"] = order_keys
    index = bisect_right(order_keys, (origin_ns, 2**63 - 1)) - 1
    if index < 0:
        return None
    row = rows[index]
    ...
    return {
        ...
        "ask": float(ask),
        "midpoint": midpoint,
        "quote_age_seconds": (origin_ns - row["sip_timestamp"]) / 1e9,
...
def route_summary(rows: list[dict[str, Any]], *, route: str) -> dict[str, Any]:
    ...
    valid = [
        row
        for row in rows
        if row.get("iv_success")
        and row.get("quote_age_seconds") is not None
        and row.get("relative_spread") is not None
    ]
    by_bucket: dict[str, list[dict[str, Any]]] = {
        name: [row for row in valid if row["bucket"] == name] for name in BUCKETS
    }
    medium = by_bucket["medium"]
    below = sorted(
        [row for row in medium if row["moneyness"] <= 1.0],
        key=lambda row: abs(row["moneyness"] - 1.0),
    )
    above = sorted(
        [row for row in medium if row["moneyness"] >= 1.0],
        key=lambda row: abs(row["moneyness"] - 1.0),
    ...
    atm = (
        (below[0]["iv"] + above[0]["iv"]) / 2.0
        if atm_interpolated
        else (below[0]["iv"] if below else above[0]["iv"] if above else None)
    )
    puts = sorted(
        [row for row in medium if row["option_type"] == "put" and row["moneyness"] <= 0.99],
        key=lambda row: abs(row["moneyness"] - 0.975),
    )
    calls = sorted(
        [row for row in medium if row["option_type"] == "call" and row["moneyness"] >= 1.01],
        key=lambda row: abs(row["moneyness"] - 1.025),
    )
    skew = puts[0]["iv"] - calls[0]["iv"] if puts and calls else None
    bucket_atm = {
        bucket: (min(values, key=lambda row: abs(row["moneyness"] - 1.0))["iv"] if values else None)
    ...
    return {
        "route": route,
        "b1q_atm_iv" if route == "B1Q" else "b1t_atm_iv": atm,
        "b1q_skew" if route == "B1Q" else "b1t_skew": skew,
        "b1q_term_structure" if route == "B1Q" else "b1t_term_structure": slopes,
        "atm_interpolated": atm_interpolated,
        "valid_contract_count": len({row["contract"] for row in valid}),
        "valid_quote_count": len(valid),
        "valid_expiry_bucket_count": sum(value is not None for value in bucket_atm.values()),
        "median_quote_age": _median([row["quote_age_seconds"] for row in valid]),
        "median_relative_spread": _median([row["relative_spread"] for row in valid]),
        "iv_inversion_success_rate": (
            sum(row.get("iv_success", False) for row in rows) / len(rows) if rows else 0.0
        ),
        "missing_reason": None if atm is not None else "NO_VALID_MEDIUM_ATM",
    }
...
def main() -> None:
    ...
    origins = pl.read_parquet(ROOT / "artifacts/pilot/b0_features.parquet").select(
        ["origin_id", "asset", "session_date", "forecast_origin_utc", "spot"]
    )
    ...
    with httpx.Client(timeout=60) as client:
        response = client.get(
            ...
            params={"from": "2026-07-01", "to": "2026-07-17", "apikey": fmp_key},
        )
    ...
    for asset in ASSETS:
        response = httpx.get(
            ...
            params={"symbol": asset, "from": "2025-07-01", "to": "2026-07-17", "apikey": fmp_key},
            timeout=60,
        )
        payload = (
            response.json()
            ...
            for dividend in rows_for_asset:
                try:
                    ...
                    if trailing_start <= declaration <= cutoff:
                        total += float(
                            dividend.get("adjDividend") or dividend.get("dividend") or 0.0
                        )
    ...
    with httpx.Client(timeout=60) as client:
        for row in date_spots.iter_rows(named=True):
            contracts[(row["asset"], row["session_date"])] = resolve_contracts(
                client, massive_key, row["asset"], row["session_date"], float(row["spot"])
            )
    jobs = [
    ...
    print(
        json.dumps(
            {"contract_day_jobs": len(jobs), "cached_files": len(list(CACHE.glob("*.json")))}
        ),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for index, future in enumerate(as_completed(futures), start=1):
            ...
            if index % 25 == 0 or index == len(futures):
                print(
                    json.dumps({"quote_jobs_completed": index, "quote_jobs_total": len(futures)}),
                    flush=True,
                )
    ...
    for (asset, day), group in sorted(grouped_origins.items()):
        ...
        for origin in group:
            ...
            for contract in contracts.get((asset, day), []):
                ...
                row = {
                    **contract,
                    ...
                    "iv_success": False,
                    "iv": None,
                    "failure_reason": "NO_QUOTE" if quote is None else quote.get("missing_reason"),
                }
                if quote and "midpoint" in quote and quote["quote_age_seconds"] > 60:
                    ...
                    quote
                    and "midpoint" in quote
                    and quote["quote_age_seconds"] <= 60
                    and quote["relative_spread"] <= 0.25
                ):
                    result = invert_iv(
                        spot,
                        contract["strike"],
                        contract["dte"] / 365.0,
                        rates.get(day, 0.0),
                        dividend_yields.get((asset, day), 0.0),
                        quote["midpoint"],
                ...
                if failure_reason == "NO_QUOTE":
                    ...
                    failure_reason = (
                        "ARBITRAGE_BOUND_FAILURE"
                        if failure_reason == "ARBITRAGE_BOUND"
                        else "IV_NO_CONVERGENCE"
                    )
                iv_attempt_rows.append(
                    {
                        "asset": asset,
                        ...
                        "success": bool(row["iv_success"]),
                        "failure_code": None if row["iv_success"] else failure_reason,
                        "iv": row.get("iv"),
                    }
                )
            ...
            term_available = (
                summary.get("b1q_term_structure", {}).get("short_to_medium") is not None
            )
            rows.append(
                {
                    "origin_id": origin["origin_id"],
                    ...
                    if origin["forecast_origin_utc"].hour == 13
                    and origin["forecast_origin_utc"].minute < 50
                    else "last"
                    if origin["forecast_origin_utc"].hour >= 19
                    else "middle",
                    **summary,
                    "b1q_atm_iv": summary.get("b1q_atm_iv"),
                    "b1q_skew": summary.get("b1q_skew"),
                    "b1q_term_structure": summary.get("b1q_term_structure"),
                    "b1q_complete": atm_available,
                    "b1a_complete": atm_available,
                    "b1b_complete": atm_available and skew_available,
                    "b1c_complete": atm_available and skew_available and term_available,
                    "b1t_atm_iv": None,
                    "b1t_skew": None,
                    "b1t_term_structure": None,
                    "b1t_complete": False,
                    "iv_attempts": len(iv_rows),
                    "iv_successes": sum(row["iv_success"] for row in iv_rows),
                    "dividend_yield": dividend_yields.get((asset, day), 0.0),
                    "route_status": "B1Q_MASSIVE_PRIMARY",
                }
            )
    ...
    segments = (
        frame.group_by(["session_segment"])
        .agg([pl.len().alias("origins"), pl.col("b1q_complete").mean().alias("b1q_atm_coverage")])
        .sort("session_segment")
    )
    segments.write_csv(OUT / "b1_coverage_by_session_segment.csv")
    summary = {
        "status": "B1Q_MASSIVE_FULL_ORIGIN_EXPLORATORY",
        "origins": frame.height,
        "assets": ASSETS,
        "coverage": {
            "b1a": frame["b1q_complete"].mean(),
            "b1b": frame.filter(pl.col("b1q_skew").is_not_null()).height / frame.height,
            "b1c": frame.filter(
                pl.col("b1q_atm_iv").is_not_null()
                & pl.col("b1q_skew").is_not_null()
                & pl.col("b1q_term_structure").struct.field("short_to_medium").is_not_null()
            ).height
            / frame.height
            if frame.height
            else 0.0,
        ...
        "thresholds": {
            ...
            "sensitivity_quote_age_seconds": 300,
            "sensitivity_relative_spread": 0.50,
        },
        "cache_contract_days": len(jobs),
        "full_backfill": False,
        "usable_for_primary": False,
        "note": "B1T and dividend-aware rerun remain required before acceptance.",
    }
    (OUT / "b1_coverage_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (OUT / "b1q_vs_b1t_comparison.csv").write_text(
        "route,origins,b1a_coverage,b1b_coverage,b1c_coverage,usable_for_primary\nB1Q,"
        + str(frame.height)
        + ","
        + str(summary["coverage"]["b1a"])
        + ","
        + str(summary["coverage"]["b1b"])
        + ","
        + str(summary["coverage"]["b1c"])
        + ",false\nB1T,0,0,0,0,false\n",
        encoding="utf-8",
    )
    (OUT / "iv_inversion_diagnostics.json").write_text(
        json.dumps(
            {
                "status": "B1Q_IV_DIAGNOSTICS",
                "attempts": int(frame["iv_attempts"].sum()),
                "successes": int(frame["iv_successes"].sum()),
                "failure_counts": failure_counts,
                "rates_source": "FMP stable treasury-rates month3 before origin",
                "dividend_source": "FMP stable dividends declarationDate <= origin, trailing 365-day sum divided by spot",
                "future_rates_or_dividends_used": False,
                "bsm_american_approximation": True,
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    pl.DataFrame(iv_attempt_rows, infer_schema_length=None, strict=False).write_csv(
        forensic_dir / "iv_failures.csv"
    )
    print(
        json.dumps(
            {
                "origins": frame.height,
                "contract_days": len(jobs),
                "b1a": summary["coverage"]["b1a"],
                "b1b": summary["coverage"]["b1b"],
                "b1c": summary["coverage"]["b1c"],
            }
        )
    )
