...
## Corpus Check
- 182 files · ~405,435 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1601 nodes · 2124 edges · 148 communities (131 shown, 17 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 153 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a1ab7b68`
...
- `CorporateEvent` --uses--> `CorporateEvent`  [INFERRED]
  tests/unit/test_event_joins.py → src/mds650/contracts.py
- `ForecastOrigin` --uses--> `CorporateEvent`  [INFERRED]
  tests/unit/test_event_joins.py → src/mds650/contracts.py
...
Cohesion: 0.07
Nodes (36): null, number, string, type, type, maximum, minimum, type (+28 more)
...
Cohesion: 0.11
Nodes (21): ForecastOrigin, Five-minute forecast origin with an explicit predictor cutoff., Reject an origin outside the ratified candidate universe., Render the origin boundary in ``America/New_York``., Prevent predictors from being available after the origin boundary., PITError, Raised when point-in-time availability cannot be established., eligible_earnings_events() (+13 more)
...
Cohesion: 0.06
Nodes (32): $ref, type, type, $ref, pattern, type, $ref, type (+24 more)
...
Cohesion: 0.09
Nodes (24): CorporateEvent, Structured corporate event with an explicit timestamp precision label., One-minute underlying OHLCV bar with an auditable deduplication key., Render the canonical bar start in ``America/New_York``., Reject assets outside the ratified candidate universe., Require finite positive OHLC values., Ensure high/low contain the open, close, and each other., UnderlyingBar (+16 more)

### Community 15 - "massive.py"
Cohesion: 0.19
Nodes (23): OptionQuote, OptionTrade, Directed historical option trade preserving provider condition codes., Directed consolidated bid/ask observation, retaining empty quotes., Reject an inverted bid/ask pair when both sides are present., Raised when a provider response changes its required schema., SchemaDriftError, _conditions() (+15 more)
...
Cohesion: 0.12
Nodes (16): authorized_for_backfill, b1_status, common_history_status, fixture_data_only, frozen_by, historical_provider_backfill, manifest_schema_version, mode (+8 more)
...
Cohesion: 0.25
Nodes (8): type, properties, items, type, blocker, evidence, status, type
...
Cohesion: 0.08
Nodes (31): ForecastOrigin, _as_float(), normalize_underlying_bars(), Provider-row normalization with explicit schema and duplicate gates., Normalize FMP one-minute OHLCV rows without silent repair.      Parameters, compute_realized_variance(), Deterministic realized-variance target construction., Compute non-annualized RV from one anchor and future one-minute closes.      P (+23 more)
...
Cohesion: 0.05
Nodes (42): applicable, blocked, conversion, fail, invalid_response, name, not_applicable, not_verified (+34 more)
...
Cohesion: 0.13
Nodes (29): main(), materialize(), _payloads(), Any, Path, Materialize the completed frozen-window provider raw run without networking., Read one complete raw run and write normalized backfill tables., Materialize the requested raw run and print a sanitized summary. (+21 more)

### Community 43 - "SchemaDriftError"
Cohesion: 0.10
Nodes (25): BaseSettings, Exception, Validated, presence-only configuration for bounded research runs., Environment-backed settings with fail-closed research safety.      Notes, Reject configurations that could enable external mutations.          Raises, Return provider-key presence without exposing values.          Returns, Fail closed unless all three provider keys are present.          Raises, ResearchSettings (+17 more)
...
Cohesion: 0.15
Nodes (15): BaseModel, _event(), _raw_bars(), test_pilot_builds_all_assets_origins_targets_and_trace(), test_pilot_fails_if_one_candidate_is_absent(), OptionStateSnapshot, ProvenanceRecord, Validated records for the six point-in-time data components. (+7 more)
...
Cohesion: 0.11
Nodes (14): ProviderHTTPClient, Small retrying HTTP client that keeps provider keys out of URLs., Create a bounded client using an Authorization header.          Parameters, Close the underlying HTTP connection pool., MassiveProvider, Request a bounded historical trade window for one contract., Request a bounded historical quote window for one contract., Close the provider connection pool. (+6 more)
...
Cohesion: 0.45
Nodes (12): candidate_assets, covered_assets, frozen_assets, AAPL, AMZN, META, MSFT, NVDA (+4 more)
...
Cohesion: 0.10
Nodes (21): Keep optional event timestamps timezone-aware when supplied., Require timezone information for event and availability timestamps., Require aware timestamps for observed and available option state., Require provider precision to remain timezone-aware., Require an aware timestamp while preserving sub-second precision., Require UTC-aware origin and cutoff timestamps., Validated primary 30-minute realized-variance target record., Require future close boundaries to preserve timezone information. (+13 more)

### Community 107 - "Any"
Cohesion: 0.50
Nodes (4): blockers, B1_NOT_AUTHORIZED, COMMON_HISTORY_NOT_ESTABLISHED, PROVIDER_FAILURES_PRESENT
...
Cohesion: 0.29
Nodes (6): Auditoría de completitud de la iteración de recuperación, Decisiones requeridas antes de la siguiente fase, Estados, Matriz requisito → evidencia, Puertas de detención vigentes, Verificación ejecutada
...
Cohesion: 0.14
Nodes (20): AssetQuality, AssetQuality, freeze_assets(), FrozenAssetSet, Quality-only asset freezing and common-history calculation., Audited coverage and data-quality measurements for one candidate asset., Reject impossible rates and inverted history ranges., Deterministic frozen assets and their maximum common history. (+12 more)
...
## Knowledge Gaps
- **649 isolated node(s):** `Gate status`, `Explicit limitations`, `Executed bounded probe`, `Massive contract-format correction run`, `Official Unusual Whales ordinary-state probe` (+644 more)
...
- **Why does `properties` connect `Community 11` to `Community 97`, `Community 98`, `Community 99`, `Community 4`, `test_errors.py`, `Community 40`, `Community 41`, `Client`, `Community 13`, `providerResult`, `Community 87`, `Community 26`, `Community 61`, `Community 95`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `asset` connect `Community 4` to `Community 11`, `Community 100`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `enum` connect `Community 100` to `Community 4`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
...
- **What connects `Gate status`, `Explicit limitations`, `Executed bounded probe` to the rest of the system?**
